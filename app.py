...
def webhook():
    # Grab raw request safely
    try:
        data = request.get_json(force=True, silent=True) or {}
    except Exception:
        data = {}
    print("🔎 Incoming payload:", data, flush=True)   # log every payload

    user_lat, user_lon = None, None

    # Case A: direct "location"
    if isinstance(data.get("location"), dict):
        user_lat = data["location"].get("latitude")
        user_lon = data["location"].get("longitude")

    # Case B: location stored in variable (userLocation)
    ...
    # Case C: fallback top-level fields
    else:
